  ...
  static Future<void> _handleSubscription(BuildContext context, WidgetRef ref,
    ...
    try {
      ...
      if (newSubscriptionLink != null) {
        ...
        // 显示成功提示
        if (getSubscriptionLink == _subscriptionService.resetSubscriptionLink) {
          _showSnackbar(context, t.userInfo.subscriptionResetSuccess);
        }
      }
    } catch (e) {
      final message = getSubscriptionLink == _subscriptionService.resetSubscriptionLink
          ? "${t.userInfo.subscriptionResetError} $e"
          : e.toString();
