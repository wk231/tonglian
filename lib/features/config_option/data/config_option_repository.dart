...
abstract class ConfigOptions {
  ...
  static final globalMode = PreferencesNotifier.create<bool, bool>(
    "global-mode",
    false,
  );
  ...
  static final singboxConfigOptions = FutureProvider<SingboxConfigOption>(
      ...
      // 根据全局模式标志来决定是否应用路由规则，不依赖服务模式
      final isGlobalMode = ref.watch(globalMode);
      final rules = PlatformUtils.isDesktop || !isGlobalMode
          ? <SingboxRule>[]
          : <SingboxRule>[
        const SingboxRule(
          domains: "domain:.cn,geosite:cn", // 中国域名
          // 不使用 ip 规则，因为 geoip 数据库可能不存在
          outbound: RuleOutbound.proxy, // 强制走代理
        ),
      ];
